  ...
  const fetchRequestsByTab = async () => {
    try {
      ...
      if (response.data?.success) {
        const { nonBulkRecord = [], promotionReportDTO = [] } = response.data.data || {};
        setRequests([...nonBulkRecord, ...promotionReportDTO]);
      }
    } catch (err) {
          ...
          <TableList
            ...
            rightElements={
              <>
                ...
                <Button variant="outline" onClick={() => fetchRequestsByTab()} className=" space-x-2 ml-3">
